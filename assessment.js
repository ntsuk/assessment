(function(){
  ...
  function makeAssessment(){
     ...
     *  @param{HTMLElement} element HTMLの要素
      */
      ...
      if(userName.length === 0){ //名前が空の場合は処理は終了
        return;
      }
      //診断結果エリアの作成
     removeAllChildren(resultDivided); //結果エリアに子要素が存在した場合削除
      ...
      anchor.dataset.text = result;
      anchor.dataset.hashtags = 'あなたのいいところ診断';
      anchor.dataset.lang = 'ja';
      anchor.dataset.showCount = false;
      anchor.innerText = '診断結果をツイートする';
  ...
  */
  function assessment(userName){
    ...
    // 文字のコード番号の合計を回数数で割って添え字の数値を求める
    const answersNum = answers.length;
    const index = sumOfcharCode % answersNum;
